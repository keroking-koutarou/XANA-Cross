...
// Game objects
const gameObjects = [
    { name: 'きのこ', size: 80, collisionRadius: 40 },
...
const allGameObjects = [
    ...
    { name: 'いちご', size: 210, collisionRadius: 110, isX: true },
        ...
        skill: {
            ...
            effect: (gameState) => {
                if (gameState.fallingObjects.length > 0) {
                    ...
                    const bomb = createBomb(selectedObject.x, selectedObject.y);
                    gameState.fallingObjects[randomIndex] = bomb;
                }
                return gameState;
            }
        }
...
オリエンテーションの時間が近づいていた。へいへいに促され、あなたは一歩を踏み出した。XANAでの新生活が今、幕を開ける。`,
        images: ["assets/images/prologue_1.PNG",
...
あなたはエミリに、彼女を助ける方法を探すと約束した。彼女を現実世界に戻すために、あなたたちは再びXANAの謎を解き明かす旅に出ることを決意する。エミリの過去と彼女がこの世界に迷い込んだ経緯を調べる中で、XANAと現実世界との奇妙な繋がりが次第に明らかになっていく。`,
    ...
    },
    {
        title: "第4章: 生徒会長の野望",
        unlockScore: 12000,
        content: `GENESIS学園の平和な日々は、生徒会長shimの突然の変化によって揺らぎ始めた。これまで穏やかで公正なリーダーとして知られていたshimが、次第に権力への執着を見せ始めたのだ。
...
function resizeGame() {
    ...
    const gameRatio = isLandscape ? 4 / 3 : 3 / 4;
    ...
    const scaleFactor = Math.min(gameWidth / GAME_WIDTH, gameHeight / GAME_HEIGHT);
    gameArea.style.transform = `scale(${scaleFactor})`;
    gameArea.style.transformOrigin = 'top left';

    // ゲームエリアの高さを制限
    const maxGameHeight = Math.min(GAME_HEIGHT, windowHeight / scaleFactor);
    gameArea.style.height = `${maxGameHeight}px`;

    updateGameObjectSizes(scaleFactor);
}
function updateGameObjectSizes(scaleFactor) {
    const gameObjects = document.querySelectorAll('.game-object');
    gameObjects.forEach(obj => {
        const baseSize = parseInt(obj.dataset.baseSize || 100);
        const newSize = baseSize * scaleFactor * 0.8; // 0.8 is a size reduction factor
...
function handleTouchMove(event) {
    ...
    moveNextObject({ clientX: x, clientY: y });
}
...
function showChapter(index) {
    const chapter = globalStory[index];
    gameArea.innerHTML = '';
    ...
    contentContainer.style.backgroundColor = 'rgba(255, 255, 255, 0.8)';
    contentContainer.style.borderRadius = '10px';

    const content = document.createElement('p');
    content.innerHTML = chapter.content.replace(/([。．！？])\s*/g, '$1<br><br>');
...
function checkUnlocks() {
    playableCharacters.forEach((char, index) => {
        if (!char.unlocked && score >= char.characterUnlockScore) {
            char.unlocked = true;
            showNotification('New Character!');
        }
    });
...
function showCharacterSelectScreen() {
    ...
    playableCharacters.forEach((char, index) => {
        ...
        charButton.addEventListener('click', () => {
            playSE('select');
            showCharacterConfirmation(char);
        });
...
function showCharacterConfirmation(character) {
    const confirmationScreen = document.createElement('div');
    confirmationScreen.id = 'character-confirmation-screen';
    confirmationScreen.innerHTML = `
        <h3>${character.name}</h3>
        <p>${character.description}</p>
        <h4>スキル: ${character.skill.name}</h4>
        <p>${character.skill.description}</p>
        <button id="start-game-button">このキャラクターでプレイ</button>
        <button id="back-to-selection-button">選択画面に戻る</button>
    `;

    const startGameButton = confirmationScreen.querySelector('#start-game-button');
    startGameButton.addEventListener('click', () => startGameWithCharacter(character));

    const backToSelectionButton = confirmationScreen.querySelector('#back-to-selection-button');
    backToSelectionButton.addEventListener('click', showCharacterSelectScreen);

    gameArea.innerHTML = '';
    gameArea.appendChild(confirmationScreen);
}
...
function createGameObjectElement(id, baseSize) {
    const element = document.createElement('div');
    element.id = id;
    element.className = 'game-object';
    element.dataset.baseSize = baseSize;
    return element;
}

function updateObjectElement(element, obj) {
    const scaleFactor = gameContainer.clientWidth / GAME_WIDTH;
    const size = obj.type.size * scaleFactor * 0.8; // 0.8 is a size reduction factor
    element.style.width = `${size}px`;
    element.style.height = `${size}px`;
    element.style.left = `${obj.x - size / 2}px`;
    element.style.top = `${obj.y - size / 2}px`;
    element.style.backgroundImage = `url('assets/images/${obj.type.name}.PNG')`;
    element.style.backgroundSize = 'contain';
}


function updateNextObjectElement(id, obj, containerScaleFactor) {
    const element = document.getElementById(id) || createGameObjectElement(id);
    const scaleFactor = containerScaleFactor * 0.8; // コンテナ内でさらに小さく

    const size = obj.type.size * scaleFactor;
    element.style.width = `${size}px`;
    element.style.height = `${size}px`;
    element.style.backgroundImage = `url('assets/images/${obj.type.name}.PNG')`;
    element.style.backgroundSize = 'contain';
    ...
    element.style.transform = `translateX(-50%) scale(${1 / containerScaleFactor})`;

    const container = document.getElementById('next-objects-container');
    const existingElement = container.querySelector(`#${id}`);
    if (existingElement) {
...
function updateNextObjectsDisplay() {
    ...
    // コンテナのスケールファクターを計算
    const containerScaleFactor = calculateContainerScaleFactor();

    // 次のオブジェクトの表示を更新
    updateNextObjectElement('next-object', nextObject, containerScaleFactor);
    updateNextObjectElement('next-next-object', nextNextObject, containerScaleFactor);
...
function createNextObjectsContainer() {
    ...
    const nextLabel = createLabel('Next');
    const nextNextLabel = createLabel('After');

    container.appendChild(nextLabel);
    container.appendChild(document.createElement('div')); // プレースホルダーfor next object
    container.appendChild(nextNextLabel);
    container.appendChild(document.createElement('div')); // プレースホルダーfor next-next object
...
function calculateContainerScaleFactor() {
    const screenWidth = window.innerWidth;
    if (screenWidth <= 480) {
        return 0.5 + 0.1 * (screenWidth / 480);
    } else if (screenWidth <= 768) {
        return 0.6 + 0.2 * (screenWidth / 768);
    }
    return 1;
}
...
// 状態が変更されたときにフラグを設定
function setNeedsRendering() {
    needsRendering = true;
}

// 物理更新後やオブジェクト操作後に呼び出す
setNeedsRendering();
...
function updatePhysics(deltaTime) {
    ...
    fallingObjects.forEach(obj => {
        ...
        obj.rotation += obj.velocity.x * 0.05;
    });
}
...
function isColliding(obj1, obj2) {
    if (obj1.type.isX || obj2.type.isX) {
        ...
        return distance < (obj1.type.collisionRadius + obj2.type.collisionRadius);
    }
}
    ...
    for (let iter = 0; iter < iterations; iter++) {
        for (let i = 0; i < fallingObjects.length; i++) {
            for (let j = i + 1; j < fallingObjects.length; j++) {
                ...
                const dy = obj2.y - obj1.y;
                const distance = Math.sqrt(dx * dx + dy * dy);
                const minDistance = (obj1.type.collisionRadius + obj2.type.collisionRadius);
...
function handleBombClick(bombIndex) {
    ...
    fallingObjects = explodeBomb(bomb, fallingObjects);
    score += 100;
    updateScore();
    renderObjects();
}
...
function activateSpecialEffect() {
    showNotification('Special Effect Activated!');
    score += 1000;
    updateScore();
}
...
function createBomb(x, y) {
    return {
        type: { name: 'bomb', size: BOMB_SIZE, isBomb: true },
        x: x,
        y: y,
        rotation: 0,
        velocity: { x: 0, y: 0 },
...
function explodeBomb(bomb, fallingObjects) {
    const explosionRadius = BOMB_EXPLOSION_RADIUS;
    return fallingObjects.filter(obj => {
        const distance = Math.sqrt((obj.x - bomb.x) ** 2 + (obj.y - bomb.y) ** 2);
        return distance > explosionRadius || obj.type.isBomb;
    });
}
...
function showGallery() {
    ...
    galleryImages.forEach(image => {
        ...
        if (image.unlocked) {
            ...
            thumbnail.style.backgroundColor = '#ccc';thumbnail.textContent = '?';
        }
...
function showGameInstructions() {
    ...
    const instructions = `
        ...
        ギャラリー：
        - はーとを合成すると、特別な報酬画像がアンロックされることがあります。
        - アンロックされた画像はギャラリーで鑑賞できます。
...
function displaySaveCode(saveCode, parentElement) {
    ...
    copyButton.addEventListener('click', () => {
        navigator.clipboard.writeText(saveCode).then(() => {
            ...
            showNotification('セーブコードがコピーされました');
        }).catch(err => {
...
function handleLoad(saveCode) {
    ...
    if (gameState) {
        ...
        showNotification('ゲームのロードに成功しました！');
        showTitleScreen();
    } else {
