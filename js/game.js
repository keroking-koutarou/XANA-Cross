...
function resizeGame() {
    ...
    if (windowRatio > gameRatio) {
        newHeight = Math.min(windowHeight, GAME_HEIGHT);
        newWidth = newHeight * gameRatio;
    } else {
        newWidth = Math.min(windowWidth, GAME_WIDTH);
        newHeight = newWidth / gameRatio;
    }
    window.scaleFactor = newWidth / GAME_WIDTH;
...
function moveNextObject(event) {
    ...
    const scaleFactor = gameArea.offsetWidth / GAME_WIDTH;
    let x;
    if (event.type === 'touch') {
    ...
    // 右端までの移動を可能にする
    nextObject.x = Math.max(nextObject.type.size / 2, Math.min(x, GAME_WIDTH - nextObject.type.size / 2));
...
function handleTouchMove(event) {
    ...
    const touch = event.touches[0];
    moveNextObject({ clientX: touch.clientX, clientY: touch.clientY, type: 'touch' });
}
