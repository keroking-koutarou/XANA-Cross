...
const GAME_HEIGHT = 1067;
const OBJECT_FALL_SPEED = 2;
const MERGE_THRESHOLD = 10;

// Game objects
const gameObjects = [
    { name: 'きのこ', size: 100, collisionRadius: 48 },
    { name: 'ねずみ', size: 120, collisionRadius: 58 },
    { name: 'ひよこ', size: 150, collisionRadius: 70 },
    { name: 'にわとり', size: 170, collisionRadius: 95 },
];
...
const allGameObjects = [
    ...
    { name: 'ひよこ', size: 150, collisionRadius: 70 },
    { name: 'にわとり', size: 170, collisionRadius: 95 },
    { name: 'ぺんぎん', size: 210, collisionRadius: 100 },
    { name: 'あざらし', size: 240, collisionRadius: 115 },
    { name: 'くま', size: 300, collisionRadius: 140 },
    { name: 'いちご', size: 210, collisionRadius: 115, isX: true },
    { name: 'かえる', size: 330, collisionRadius: 150 },
    { name: 'はーと', size: 360, collisionRadius: 165, isHeart: true }
];
...
const playableCharacters = [
    ...
    },
    {
        name: 'アヒッル',
        unlocked: false,
        storyUnlockScore: 6000,
        characterUnlockScore: 7000,
    ...
    },
    {
        name: 'エレン',
        unlocked: false,
        storyUnlockScore: 8000,
        characterUnlockScore: 9000,
    ...
    },
    {
        name: 'shim',
        unlocked: false,
        storyUnlockScore: 10000,
        characterUnlockScore: 11000,
        ...
        skill: {
            name: 'なおたんのために',
            description: '現在場にあるオブジェクトの半分をランダムであざらしのオブジェクトに変化させる',
            effect: (gameState) => {
                const objectCount = gameState.fallingObjects.length;
                const changeCount = Math.floor(objectCount / 2);
                const indexesToChange = new Set();
                
                while (indexesToChange.size < changeCount) {
                    indexesToChange.add(Math.floor(Math.random() * objectCount));
                }
                ...
                gameState.fallingObjects = gameState.fallingObjects.map((obj, index) => {
                    if (indexesToChange.has(index)) {
                        return {
                            ...obj,
                            type: allGameObjects.find(type => type.name === 'あざらし')
                        };
                    }
                    return obj;
                });
    ...
    },
    {
        name: 'RIO',
        unlocked: false,
        storyUnlockScore: 12000,
        characterUnlockScore: 13000,
    ...
    },
    {
        name: 'ミンミン',
        unlocked: false,
        storyUnlockScore: 14000,
        characterUnlockScore: 15000,
        ...
        skill: {
            name: 'アヒッルマニア',
            description: '場にあるすべてのオブジェクトをひよこに変更する',
            effect: (gameState) => {
                gameState.fallingObjects = gameState.fallingObjects.map(obj => ({
                    ...obj,
                    type: allGameObjects.find(type => type.name === 'ひよこ')
                }));
                return gameState;
            }
        }
    },
    {
        name: 'マリコ',
        unlocked: false,
        storyUnlockScore: 16000,
        characterUnlockScore: 18000,
        ...
        skill: {
            name: 'けろきんぐ',
            description: '場にあるすべてのオブジェクトをかえるに変更する',
            effect: (gameState) => {
                gameState.fallingObjects = gameState.fallingObjects.map(obj => ({
                    ...obj,
                    type: allGameObjects.find(type => type.name === 'かえる')
                }));
                return gameState;
            }
        }
...
const galleryImages = [
    ...
    { id: 7, src: 'assets/images/heart_reward_7.PNG', unlocked: false },
    { id: 8, src: 'assets/images/heart_reward_8.PNG', unlocked: false },
    { id: 9, src: 'assets/images/heart_reward_9.PNG', unlocked: false },
    { id: 10, src: 'assets/images/heart_reward_10.PNG', unlocked: false }
];
...
function manageBGM(bgmKey) {
    ...
    currentBGM.volume = 0.5; // デフォルトの音量を設定
...
function showChapter(index) {
    ...
    const scrollableContent = createScrollableContent(content.outerHTML);
    chapterScreen.appendChild(scrollableContent);
...
function showCharacterInfo(character, selectScreen) {
    ...
    if (character.unlocked) {
        characterInfo.innerHTML = `
            ...
            <p class="skill-description">${character.skill.description}</p>
        ...
        characterInfo.innerHTML = `
            <h3>ロック中のキャラクター</h3>
            <p class="locked-description">このキャラクターはまだアンロックされていません。</p>
            <p class="locked-description">スコア ${character.characterUnlockScore} 以上でアンロックされます。</p>
            <button id="back-to-selection-button">キャラクター選択に戻る</button>
        `;
        document.getElementById('back-to-selection-button').addEventListener('click', () => {
            characterInfo.style.display = 'none';
    ...
    // 背景画像の設定を調整
    selectScreen.style.backgroundSize = 'cover';
    selectScreen.style.backgroundPosition = 'center';
}
...
function resizeGame() {
    ...
    const scaleFactor = newWidth / GAME_WIDTH;
    gameArea.style.transform = `scale(${scaleFactor})`;
    gameArea.style.transformOrigin = 'top left';

    // スケールファクターをグローバル変数として保存
    window.currentScaleFactor = scaleFactor;
}

function adjustObjectSizes(scaleFactor) {
    allGameObjects.forEach(obj => {
        obj.adjustedSize = obj.size * scaleFactor;
        obj.adjustedCollisionRadius = obj.collisionRadius * scaleFactor;
    });
    GAME_OVER_LINE = 100 * scaleFactor;
}
...
function createRandomObject() {
    ...
    return {
        ...
        y: GAME_OVER_LINE + allGameObjects[randomIndex].size / 2, // ゲームオーバーラインの直下に配置
        rotation: 0,
        velocity: { x: 0, y: 0 },
...
function moveNextObject(event) {
    ...
    const scaleFactor = rect.width / GAME_WIDTH;
    ...
    // オブジェクトの移動範囲を制限
    const minX = nextObject.type.size / 2;
    const maxX = GAME_WIDTH - nextObject.type.size / 2;
    nextObject.x = Math.max(minX, Math.min(x, maxX));
...
function updateNextObjectPosition() {
    ...
    nextObjectElement.style.width = `${nextObject.type.size}px`;
    nextObjectElement.style.height = `${nextObject.type.size}px`;
    nextObjectElement.style.left = `${nextObject.x - nextObject.type.size / 2}px`;
    nextObjectElement.style.top = `${nextObject.y - nextObject.type.size / 2}px`;
    nextObjectElement.style.backgroundImage = `url('assets/images/${nextObject.type.name}.PNG')`;
    nextObjectElement.style.backgroundSize = 'contain';
    nextObjectElement.style.backgroundRepeat = 'no-repeat';
...
function createGameObjectElement(id) {
    ...
    element.setAttribute('data-original-left', element.offsetLeft);
    element.setAttribute('data-original-top', element.offsetTop);
    return element;
}
...
function adjustGameElements() {
    // ゲーム内の要素のサイズを調整
    const gameElements = document.querySelectorAll('.game-object');
    gameElements.forEach(element => {
        const originalWidth = parseInt(element.getAttribute('data-original-width') || element.offsetWidth);
        const originalHeight = parseInt(element.getAttribute('data-original-height') || element.offsetHeight);
        element.style.width = `${originalWidth * scaleFactor}px`;
        element.style.height = `${originalHeight * scaleFactor}px`;
        
        // 位置も調整が必要な場合
        const originalLeft = parseInt(element.getAttribute('data-original-left') || element.offsetLeft);
        const originalTop = parseInt(element.getAttribute('data-original-top') || element.offsetTop);
        element.style.left = `${originalLeft * scaleFactor}px`;
        element.style.top = `${originalTop * scaleFactor}px`;
        
        // オリジナルのサイズと位置を保存（初回のみ）
        if (!element.hasAttribute('data-original-width')) {
            element.setAttribute('data-original-width', originalWidth);
            element.setAttribute('data-original-height', originalHeight);
            element.setAttribute('data-original-left', originalLeft);
            element.setAttribute('data-original-top', originalTop);
        }
    });

    // その他の調整が必要な要素があればここで行う
    // 例: スコア表示、ゲームオーバーライン、次のオブジェクト表示など
}
...
function dropObject() {
    ...
    const droppedObject = { 
        ...nextObject, 
        y: Math.max(nextObject.type.size / 2, GAME_OVER_LINE + nextObject.type.size / 2)
    };
...
const ROTATION_DAMPING = 0.95; // 回転の減衰係数
const VELOCITY_THRESHOLD = 0.1; // 速度のしきい値
const ROTATION_THRESHOLD = 0.1; // 回転速度のしきい値
...
function updatePhysics(deltaTime) {
    ...
    fallingObjects.forEach(obj => {
        ...
        // 回転の更新と減衰
        obj.rotation += obj.velocity.x * 0.05; // 回転速度を小さくする
        obj.rotation *= ROTATION_DAMPING;
        ...
        if (Math.abs(obj.rotation) < ROTATION_THRESHOLD) obj.rotation = 0;
    });
}
...
function renderObjects() {
    ...
    fallingObjects.forEach((obj, index) => {
        ...
        element.style.width = `${obj.type.adjustedSize}px`;
        element.style.height = `${obj.type.adjustedSize}px`;
        element.style.left = `${obj.x - obj.type.adjustedSize / 2}px`;
        element.style.top = `${obj.y - obj.type.adjustedSize / 2}px`;
...
function activateSpecialEffect() {
    showUnlockNotification('はーと生成！');
    score += 1000;
    updateScore();
}

function checkGameOver() {
    const threshold = GAME_OVER_LINE + 5; // 少し余裕を持たせる
    const staticObjects = fallingObjects.filter(obj => Math.abs(obj.velocity.y) < 0.1);
    
    if (staticObjects.some(obj => {
        const topOfObject = obj.y - obj.type.size / 2;
        return topOfObject <= threshold;
    })) {
        isGameOver = true;
...
function showGameOverScreen() {
    ...
    document.getElementById('retry-button').addEventListener('click', () => {
        gameArea.removeChild(gameOverScreen);
        resetGame();
        startGameWithCharacter(currentCharacter);
    });
...
function useSkill() {
    if (skillUses > 0 && !isGameOver) {
        ...
        const gameState = {
            ...
            score: score,
            bombs: bombs
            // shimSkillTurns と marikoSkillTurns は新しいスキル効果では不要なので削除
        };
        ...
        // shimSkillTurns と marikoSkillTurns の更新を削除
...
function updateSkillButton() {
    ...
    skillButton.addEventListener('touchstart', handleSkillButtonTouch);
}

function handleSkillButtonTouch(event) {
...
function displayUnlockMessage(message) {
    ...
    if (message === 'New Character!') {
        notification.innerHTML = 'New<br>Character!';
    } else {
        notification.textContent = message;
    }
    document.body.appendChild(notification);
...
function showStory() {
    ...
    globalStory.forEach((chapter, index) => {
        if (chapter.unlocked) {
            content += createChapterSection(chapter);
        } else {
            content += `<div class="locked-chapter">スコア${chapter.unlockScore}でアンロック</div>`;
        }
    });

    const scrollableContent = createScrollableContent(content);
    storyContent.appendChild(scrollableContent);
...
function setupScrollButton(container) {
    const scrollButton = container.querySelector('#scroll-button');
    const content = container.querySelector('.content');
    
    let scrollInterval;
    
    scrollButton.addEventListener('touchstart', startScroll);
    scrollButton.addEventListener('mousedown', startScroll);
    scrollButton.addEventListener('touchend', stopScroll);
    scrollButton.addEventListener('mouseup', stopScroll);
    scrollButton.addEventListener('mouseleave', stopScroll);
    
    function startScroll(e) {
        e.preventDefault();
        scrollInterval = setInterval(() => {
            content.scrollTop += 5; // スクロール速度を調整
        }, 20);
    }
...
function createScrollableContent(content) {
    const container = document.createElement('div');
...
function showGameInstructions() {
    ...
    const instructions = `
        <h3>「XANA/クロスオーバーパズル」へようこそ！</h3>
        <p>ここは仮想の世界、XANAメタバース。様々な不思議な出会いと冒険があなたを待っています。</p>

        <h4>【ゲームの目的】</h4>
        <p>XANAの世界で様々なオブジェクトを合体させ、高得点を目指しながら、謎に満ちた物語を解き明かしていきます。</p>

        <h4>【基本的な遊び方】</h4>
        <ol>
            <li>画面上部から落ちてくるオブジェクトを操作します。</li>
            <li>オブジェクトを左右に移動させ、タップorクリックで落下させます。</li>
            <li>同じ種類のオブジェクト同士が接触すると、より大きな新しいオブジェクトに進化します。</li>
            <li>オブジェクトを進化させるほど高いスコアを獲得できます。</li>
        </ol>

        <h4>【ゲームオーバー】</h4>
        <p>積み上がったオブジェクトが画面上部の赤いラインに到達するとゲームオーバーです。<br>
        ゲームオーバー後は、獲得したスコアに応じて様々な要素がアンロックされます。</p>

        <h4>【キャラクター】</h4>
        <ul>
            <li>ゲームを進めると、個性豊かな新しいキャラクターがアンロックされます。</li>
            <li>各キャラクターには固有のスキルがあり、1ゲームにつき2回、もしくは1回使用できます。</li>
            <li>キャラクターのスキルを戦略的に使用することで、より高いスコアを狙えます。</li>
        </ul>

        <h4>【ストーリー】</h4>
        <ul>
            <li>スコアを獲得すると、新しいストーリーチャプターが解放されます。</li>
            <li>ストーリー画面では、XANAの世界観や登場人物たちの物語を楽しむことができます。</li>
        </ul>

        <h4>【ギャラリー】</h4>
        <ul>
            <li>ゲーム中に特定の条件を満たすと、特別な報酬画像がアンロックされます。</li>
            <li>アンロックされた画像はギャラリーで鑑賞することができます。</li>
            <li>すべての画像を集めると、XANAの世界についての新たな発見があるかも（ないかも）しれません。</li>
        </ul>

        <h4>【セーブ＆ロード】</h4>
        <ul>
            <li>ゲームの進行状況は自動的に保存されます。</li>
            <li>セーブ＆ロード画面では、現在の進行状況をコードとして出力できます。</li>
            <li>このコードを使用することで、別のデバイスでも同じ進行状況からゲームを再開できます。</li>
            <li>大切な進行状況は、こまめにコードを保存しておくことをおすすめします。</li>
        </ul>

        <h4>【ヒント】</h4>
        <ul>
            <li>キャラクターのスキルを効果的に使うことで、より高いスコアを狙えます。</li>
            <li>ストーリーを進めることで、ゲームの世界観がより深く理解できるでしょう。</li>
            <li>ギャラリーのコンプリートを目指すことで、より多くのやりこみ要素を楽しめます。</li>
        </ul>

        <p class="conclusion">さあ、XANAの不思議な世界で、パズルとストーリーを楽しみましょう！<br>
        あなたの冒険が、今始まります。</p>
    `;

    const scrollableContent = createScrollableContent(instructions);
...
function showSaveLoadScreen() {
    ...
    // 背景画像の設定
    saveLoadScreen.style.backgroundImage = 'url("assets/images/save_load_background.jpg")';
    saveLoadScreen.style.backgroundSize = 'cover';
    saveLoadScreen.style.backgroundPosition = 'center';
    ...
    saveButton.addEventListener('click', () => {
        ...
        if (saveCode) {
            displaySaveCode(saveCode, contentWrapper);
    ...
    saveLoadScreen.appendChild(contentWrapper);
...
function displaySaveCode(saveCode, parentElement) {
    ...
    saveCodeText.textContent = 'あなたのセーブコード:';
    saveCodeDisplay.appendChild(saveCodeText);

    const codeContainer = document.createElement('div');
    codeContainer.className = 'save-code-container';
    // セーブコードを20文字ごとに分割
    const codeLines = saveCode.match(/.{1,20}/g);
    codeLines.forEach(line => {
        const lineElement = document.createElement('span');
        lineElement.textContent = line;
        codeContainer.appendChild(lineElement);
    });
    saveCodeDisplay.appendChild(codeContainer);
...
function showLoadInput(parentElement) {
    ...
    const existingLoadInput = document.getElementById('load-input-container');
...
function handleTouchMove(event) {
    event.preventDefault();
    const touch = event.touches[0];
    moveNextObject({ clientX: touch.clientX, clientY: touch.clientY, isTouch: true });
}
...
function setupScrollButton() {
    ...
    scrollButton.innerHTML = '&#10095;&#10095;'; // 二重の右向き矢印
    scrollButton.style.transform = 'rotate(90deg)'; // 90度回転させて下向きにする
    scrollableContent.appendChild(scrollButton);
...
function createScrollableContent(content) {
    ...
    upButton.innerHTML = '&#9650;'; // 上向き三角形
    buttonContainer.appendChild(upButton);
    
    const downButton = document.createElement('button');
    downButton.className = 'scroll-button down';
    downButton.innerHTML = '&#9660;'; // 下向き三角形
...
function setupScrollButtons(container) {
    const content = container.querySelector('.content');
    const upButton = container.querySelector('.scroll-button.up');
    const downButton = container.querySelector('.scroll-button.down');
    
    let scrollInterval;
    
    function startScroll(direction) {
        scrollInterval = setInterval(() => {
            content.scrollTop += direction * 5; // スクロール速度を調整
        }, 20);
    }
    ...
    upButton.addEventListener('mousedown', () => startScroll(-1));
    
    downButton.addEventListener('touchstart', (e) => {
        e.preventDefault();
        startScroll(1);
    });
    downButton.addEventListener('mousedown', () => startScroll(1));
    ...
    content.addEventListener('scroll', () => {
        upButton.style.display = content.scrollTop > 0 ? 'block' : 'none';
