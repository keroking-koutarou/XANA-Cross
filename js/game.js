...
function resizeGame() {
    ...
    if (windowRatio > gameRatio) {
        scaleFactor = windowHeight / GAME_HEIGHT;
    } else {
        scaleFactor = windowWidth / GAME_WIDTH;
    }
...
function isColliding(obj1, obj2) {
    if (obj1.type.isX || obj2.type.isX) {
        ...
        return distance < (obj1.type.collisionRadius + obj2.type.collisionRadius) * 1.1;
    }
}
    ...
    for (let iter = 0; iter < iterations; iter++) {
        for (let i = 0; i < fallingObjects.length; i++) {
            for (let j = i + 1; j < fallingObjects.length; j++) {
                ...
                const dy = obj2.y - obj1.y;
                const distance = Math.sqrt(dx * dx + dy * dy);
                const minDistance = (obj1.type.collisionRadius + obj2.type.collisionRadius) * 0.95;
...
function renderObjects() {
    ...
    fallingObjects.forEach((obj, index) => {
        ...
        element.style.width = `${obj.adjustedSize || obj.size}px`;
        element.style.height = `${obj.adjustedSize || obj.size}px`;
...
function createScrollableContent(content) {
    ...
    indicator.textContent = '下にスクロールして続きを読む';
